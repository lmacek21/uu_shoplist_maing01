...
const BeingInitializedAuthorized = createVisualComponent({
  ...
  render() {
    ...
    const { data: uuShoplist } = useSubAppData();
    ...
    if (uuShoplist?.temporaryData?.dtoIn.mode) {
      uuShoplist.temporaryData.dtoIn.mode = "retry";
    }

    const [showConsole, setShowConsole] = useState(!!uuShoplist.consoleCode);
    const [retryInputData, setRetryInputData] = useState(
      JSON.stringify({ force: false, mode: "retry", data: uuShoplist?.temporaryData?.dtoIn }, null, 2),
    );
    const [rollbackInputData, setRollbackInputData] = useState(
      JSON.stringify({ force: true, mode: "rollback", data: {} }, null, 2),
    );
    const consoleBaseUri = uuShoplist.temporaryData.progressMap.uuConsoleUri;
    const progressCode = uuShoplist.temporaryData.progressMap.progressCode;
    const consoleCode = uuShoplist.temporaryData.progressMap.consoleCode;
