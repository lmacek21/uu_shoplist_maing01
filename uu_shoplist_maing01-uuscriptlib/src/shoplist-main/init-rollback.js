...
const Names = {
  ...
  SCRIPT_NAME: "ShoplistMainInitRollback",
};
/*@@viewOff:names*/
...
/*@@viewOn:scriptClient*/
class ShoplistMainInitRollbackClient {
  ...
  async initFinalizeRollback() {
    return this.shoplistMainClient.initFinalizeRollback(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptShoplistMainInitRollbackDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptShoplistMainInitRollbackDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuShoplist initRollback started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new ShoplistMainInitRollbackClient(dtoIn.lockSecret);
  ...
  uuShoplist = await mainContext.initFinalizeRollback();
