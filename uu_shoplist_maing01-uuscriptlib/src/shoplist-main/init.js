...
class ShoplistMainInitClient {
  ...
  async initFinalize() {
    return this.shoplistMainClient.initFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptShoplistMainInitDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptShoplistMainInitDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuShoplist init started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new ShoplistMainInitClient(dtoIn.lockSecret);
  ...
  uuShoplist = await mainContext.initFinalize();
