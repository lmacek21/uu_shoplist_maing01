...
class ShoplistMainClearClient {
  ...
  async clearFinalize() {
    return this.shoplistMainClient.clearFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptShoplistMainClearDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptShoplistMainClearDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuShoplist clear started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new ShoplistMainClearClient(dtoIn.lockSecret);
  ...
  uuShoplist = await mainContext.clearFinalize();
