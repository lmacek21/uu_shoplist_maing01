...
const ShoplistMainConstants = require("../../constants/shoplist-main-constants.js");
...
class ShoplistMainUseCaseError extends UseCaseError {
  constructor(code, message, paramMap, cause, status = 400, dtoOut) {
    ...
    if (!dtoOut) {
      dtoOut = { uuAppErrorMap: UseCaseContext.getAttribute(ShoplistMainConstants.UU_APP_ERROR_MAP) };
    }
  ...
  static generateCode(code) {
    ...
    return code.indexOf("/") < 0 ? `${ShoplistMainConstants.ERROR_PREFIX}/${uri.getUseCase()}/${code}` : code;
  }
}
